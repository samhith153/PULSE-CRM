"""
Intent Detection Repository
Fetches aggregated and per-row interaction text from
activity_timeline_events and emails.
Indexed columns used: entity_id, entity_type, created_at, owner_id, sent_at.
No N+1 — string_agg per lead in one query, detail rows on demand.
"""
...
class IntentRepository:
    ...
    # ── Aggregated text per lead ──────────────────────────────────────────────

    async def fetch_lead_texts(
        self,
    ...
    # ── Email text for a lead ─────────────────────────────────────────────────

    async def fetch_lead_email_text(
        self,
        organization_id: UUID,
        lead_id: UUID,
    ) -> str:
        stmt = select(
            func.string_agg(
                Email.subject + " " + func.coalesce(Email.body_preview, ""),
                " ",
            )
        ).where(
            Email.organization_id == organization_id,
