"""
CRM Call Model
Represents a logged sales call linked to any CRM entity.
"""
...
class CrmCall(Base, TenantMixin):
    """A sales call logged by a CRM user."""
    ...
    call_type: Mapped[str] = mapped_column(String(20), nullable=False, default="outbound", index=True)
    # inbound | outbound

    duration_minutes: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    outcome: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)
    # connected | busy | left_vm | call_back_later | no_answer
    ...
    def __repr__(self) -> str:
        return f"<CrmCall id={self.id} subject={self.subject!r} outcome={self.outcome!r}>"
