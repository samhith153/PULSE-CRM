...
function WorkflowCard({ item, index }: { item: typeof WORKFLOW[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      style={{ background: '#fff', borderRadius: 16, border: `1px solid ${hovered ? '#ddd6fe' : '#eaecef'}`, padding: '20px 16px 18px', cursor: 'pointer', transition: 'all 0.18s ease', transform: hovered ? 'translateY(-3px)' : 'translateY(0)', boxShadow: hovered ? '0 10px 28px rgba(37,99,235,0.10)' : '0 1px 4px rgba(0,0,0,0.04)' }}
    >
  ...
  return (
    <PageContainer>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', top: -100, right: -100, width: 480, height: 480, background: 'radial-gradient(circle, rgba(37,99,235,0.07) 0%, transparent 65%)', pointerEvents: 'none' }} />
