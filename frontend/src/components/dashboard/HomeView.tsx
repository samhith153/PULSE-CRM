                              ...
                              <tbody className="divide-y divide-border/30 text-xs font-semibold text-text-primary">
                                {sortedTasks.map((task) => {
                                  ...
                                  const scoreColor = score === null ? '' :
                                    score >= 75 ? 'bg-status-success/10 text-status-success dark:text-status-success border-status-success/20' :
                                    score >= 50 ? 'bg-status-warning/10 text-status-warning dark:text-status-warning border-status-warning/20' :
                                    'bg-status-danger/10 text-status-danger dark:text-status-danger border-status-danger/20';
                                  return (
                                  <tr key={task.id} className="hover:bg-surface-2/15 transition-colors">
                                    ...
                                    <td className="py-[var(--space-2)] px-[var(--space-3)] text-right whitespace-nowrap">
                                      <span 
                                        className={`px-2 py-0.5 rounded text-[10px] font-bold inline-block border ${
                                          task.status === 'Overdue' ? 'bg-status-danger/10 text-status-danger border-status-danger/15' :
                                          task.status === 'Completed' ? 'bg-status-success/10 text-status-success border-status-success/15' :
                                          task.status === 'In Progress' ? 'bg-accent-color/10 text-accent-color border-accent-color/15' :
