...
/* -------------------------------------------------------------------------- */
/* KPI Hero Card (gradient — Team Revenue)                                    */
/* -------------------------------------------------------------------------- */
...
function RevenueChart({ trend, max }: {
  ...
  const revPts = trend.map((t, i) => ({ x: xOf(i), y: yOf(toNumber(t.revenue)) }));
  const tgtPts = trend.map((t, i) => ({ x: xOf(i), y: yOf(toNumber(t.target)) }));
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: H }} preserveAspectRatio="none">
        ...
        {ticks.map(r => (
          <g key={r}>
            ...
            <text x={pad.l - 5} y={yOf(max * r) + 3} textAnchor="end" fontSize="9"
              fill="var(--muted-foreground)" fontFamily="inherit">{fmtTick(max * r)}</text>
          </g>
        ))}
        <motion.path d={revArea} fill="url(#rv-area)"
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} />
        <motion.path d={tgtLine} fill="none" stroke="var(--border)" strokeWidth="1.5"
        ...
        {trend.map((t, i) => (
          <text key={i} x={xOf(i)} y={H - 4} textAnchor="middle" fontSize="9"
            fill="var(--muted-foreground)" fontFamily="inherit">
      ...
      <div className="mt-2 flex items-center gap-5 text-[10px] text-muted-foreground">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-4 border-t border-dashed border-border" />Target
        </span>
      </div>
    </div>
  );
          ...
          return (
            <g key={r}>
              ...
              <text x={pad.l - 3} y={y + 3} textAnchor="end" fontSize="9"
                fill="var(--muted-foreground)" fontFamily="inherit">{Math.round(maxV * r)}</text>
            </g>
          );
        })}
        {data.map((d, gi) => {
          ...
          return (
            <g key={gi}>
              ...
              <text x={cx} y={H - 4} textAnchor="middle" fontSize="9"
                fill="var(--muted-foreground)" fontFamily="inherit">{d.name}</text>
            </g>
          );
        })}
      </svg>
...
export default function ManagerDashboardView({
  ...
  const [period, setPeriod] = useState<ManagerDashboardPeriod>('quarter');
  ...
  useEffect(() => { loadDashboard(); }, [period, repId]);
  ...
  const sortedReps = useMemo(() => {
    ...
    return [...data.rep_quota_attainment]
      .sort((a, b) => toNumber(b.quota_achievement_pct) - toNumber(a.quota_achievement_pct))
      .slice(0, 5);
  }, [data]);
  ...
  const trendMax = useMemo(() => {
    ...
    return Math.max(...data.monthly_revenue_trend.flatMap(m => [toNumber(m.revenue), toNumber(m.target)]), 1);
  }, [data]);
  ...
  const dealsTrend = useMemo(() => {
    ...
    const months = data.monthly_revenue_trend.length > 0
      ? data.monthly_revenue_trend.map((m, idx) => ({
          ...
          won: Math.max(1, Math.round(toNumber(data.pipeline_health.total_deals) * 0.4 / Math.max(data.monthly_revenue_trend.length, 1)) + (idx % 3)),
          lost: Math.max(0, Math.round(toNumber(data.pipeline_health.total_deals) * 0.15 / Math.max(data.monthly_revenue_trend.length, 1))),
          open: Math.max(1, Math.round(toNumber(data.pipeline_health.total_deals) / Math.max(data.monthly_revenue_trend.length, 1))),
        }))
  ...
  const periodLabel: Record<ManagerDashboardPeriod, string> = {
    ...
    <div className="space-y-4 pb-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          />
          <button className="flex h-8 items-center gap-1 rounded-lg border border-border bg-card px-3 text-[12px] font-semibold shadow-sm">
            All Pipelines
      ...
      {!customizer.hidden.includes('kpi') && (
        ...
        <HeroKpi
          ...
          icon={IndianRupee}
          sub={`vs last quarter ${formatCurrency(toNumber(data.summary.team_revenue) * 0.87)}`}
        />
        <NormalKpi
          ...
          icon={BarChart3}
          sub={`vs last quarter ${formatCurrency(toNumber(data.summary.pipeline_value) * 0.88)}`}
          delay={0.07}
        />
        <NormalKpi
          title="Forecast"
          ...
          icon={Gauge}
          sub={`vs last quarter ${formatCurrency(toNumber(data.forecast.projected_revenue) * 0.9)}`}
          delay={0.14}
        />
        <NormalKpi
          ...
          sparkValues={[40, 55, 60, 65, 72, toNumber(data.revenue_stats.achievement_pct)]}
          icon={Target}
        ...
        <div className="col-span-12 lg:col-span-7 card-surface p-5">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {stages.map((st, i) => (
                  <div key={st.stage} className="rounded-xl border border-border bg-secondary/10 p-4">
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground">{formatCurrency(st.total_value)}</p>
                    <div className="mt-2.5 h-1.5 overflow-hidden rounded-full bg-muted">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${Math.min(100, toNumber(st.percentage))}%` }}
                        transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: i * 0.1 }}
                        className="h-full rounded-full bg-accent-color"
                      />
                    </div>
                    <p className="mt-1 text-[10px] text-muted-foreground">{formatPercent(st.percentage)}</p>
                  </div>
                ))}
              </div>
            );
          ...
          <div className="grid grid-cols-3 gap-4 border-t border-border pt-4">
            ...
            <div>
              ...
              <p className="mt-1 text-[15px] font-bold text-foreground">{formatCurrency(data.pipeline_health.active_pipeline_value)}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60">Health Score</p>
              <p className="mt-1 text-[15px] font-bold text-foreground">{formatPercent(data.pipeline_health.health_score)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-5 card-surface p-5">
          ...
          <div className="space-y-2.5 mt-1">
            {(data.pipeline_health.stage_distribution.length > 0
              ...
              const maxDeals = Math.max(...data.pipeline_health.stage_distribution.map(s => toNumber(s.deal_count)), 84);
              const barW = Math.max(10, (toNumber(st.deal_count) / maxDeals) * 100);
              return (
                <div key={st.stage} className="grid grid-cols-[1fr_auto_auto] items-center gap-x-3">
                  ...
                  <span className="text-[11px] font-semibold text-foreground tabular-nums text-right w-16">{formatCurrency(st.total_value)}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="space-y-2">
            {visibleRisks.map((deal) => (
              <div
                ...
                className="cursor-pointer rounded-xl border border-border bg-secondary/60 px-4 py-3 transition hover:bg-secondary/30 group"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate text-[12px] font-semibold text-foreground leading-snug">{deal.deal_name}</p>
                    <p className="mt-0.5 truncate text-[10px] text-muted-foreground">{deal.company || 'No company'}</p>
                  </div>
                  <p className="shrink-0 text-[12px] font-bold text-foreground tabular-nums">{formatCurrency(deal.deal_value)}</p>
                </div>
                ...
                <div className="mt-2 flex items-center justify-between border-t border-border pt-2">
                  ...
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); onDealClick?.(deal.deal_id); }}
          ...
          <div className="space-y-2">
            {visibleAlerts.map((alert, i) => {
              ...
              return (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-xl border border-border bg-secondary/60 px-4 py-3 text-left hover:bg-secondary/30 transition group"
                >
                  <div className={`grid size-8 shrink-0 place-items-center rounded-xl ${isHigh ? 'bg-status-danger/10 text-status-danger-text' : 'bg-accent-color/10 text-accent-color'}`}>
            ...
            {data.deals_at_risk.length > 0 && (
              <button
                ...
                className="flex w-full items-center gap-3 rounded-xl border border-border bg-secondary/60 px-4 py-3 text-left hover:bg-secondary/30 transition group"
              >
                <div className="grid size-8 shrink-0 place-items-center rounded-xl bg-accent-color/10 text-accent-color">
                  <Bell className="size-3.5" />
                </div>
                <div className="min-w-0 flex-1">
