...
import { summarizeThread, ConversationSummary, SummaryMessage, getEmails } from '@/utils/api';
...
export default function EmailsView() {
  ...
  useEffect(() => {
    getEmails({ page_size: 100 })
      .then(res => {
        const list = Array.isArray(res) ? res : (res?.data ?? []);
        const mapped = list.map((m: any) => ({
          id: m.id,
          sender: m.sender ? m.sender.split('<')[0].trim() : 'Unknown',
          senderEmail: m.sender ? (m.sender.match(/<([^>]+)>/)?.[1] || m.sender) : '',
          ...
          unread: !m.is_read,
          starred: false,
          attachments: (m.attachment_metadata || []).map((a: any) => ({
            name: a.filename || 'attachment',
            size: a.size_bytes ? `${(a.size_bytes / 1024).toFixed(0)} KB` : 'Unknown'
          }))
        }));
        setThreads(mapped);
      })
  ...
  // Selection state
  const [selectedThreads, setSelectedThreads] = useState<(number | string)[]>([]);
  ...
  const handleToggleStar = (e: React.MouseEvent, id: number | string) => {
    e.stopPropagation();
    setThreads(threads.map(t => t.id === id ? { ...t, starred: !t.starred } : t));
  };

  const handleSelectThread = (e: React.ChangeEvent<HTMLInputElement>, id: number | string) => {
    e.stopPropagation();
    if (e.target.checked) {
  ...
  const handleRowDelete = (e: React.MouseEvent, id: number | string) => {
    e.stopPropagation();
  ...
  const handleRowToggleRead = (e: React.MouseEvent, id: number | string) => {
    e.stopPropagation();
