...
export default function CommandPalette({ isOpen, onClose, setActiveTab, onNewReportClick }: CommandPaletteProps) {
  ...
  return (
    <div 
      className="fixed inset-0 bg-ink/40 backdrop-blur-xs z-50 flex items-start justify-center pt-24 px-4 transition-all duration-300 animate-in fade-in"
      onClick={handleBackdropClick}
    >
      <div className="bg-card border border-border w-full max-w-md rounded-xl  overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[380px]">
        {/* Search header bar */}
        <div className="relative border-b border-border flex items-center shrink-0">
          ...
          <input
            ...
            className="w-full pl-11 pr-20 py-3.5 text-xs text-muted-foreground bg-card placeholder-slate-400 focus:outline-none"
          />
        ...
        >
          {query.trim() === '' ? (
            <div className="py-6 flex flex-col items-center justify-center text-center space-y-2">
              <div className="p-2 bg-brand-purple/5 rounded-full border border-border text-brand-purple/60">
                <Search className="h-5 w-5" strokeWidth={1.5} />
              </div>
              <div className="space-y-0.5">
                <p className="text-xs font-bold text-muted-foreground">Search Pulse CRM</p>
                <p className="text-[10px] text-muted-foreground font-semibold max-w-[240px]">Search pages, leads, contacts, companies, and deals...</p>
              </div>
            </div>
          ) : filtered.length > 0 ? (
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-left select-none cursor-pointer transition-all duration-150 ${
                    isActive 
                      ? 'bg-brand-purple/10 text-brand-purple border-l-3 border-brand-accent pl-2' 
                      : 'text-muted-foreground/75 hover:bg-secondary hover:text-muted-foreground border-l-3 border-transparent'
                  }`}
                >
                  <div className="flex items-center space-x-3 min-w-0">
                    <div className={`p-1.5 rounded-md ${
                      isActive ? 'bg-brand-purple/15 text-brand-purple' : 'bg-secondary/80 text-slate-550'
                    }`}>
                  ...
                  <div className="flex items-center space-x-2 shrink-0">
                    <span className={`text-[9px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded border ${
                      isActive 
                        ? 'bg-brand-purple/10 border-brand-accent/25 text-brand-purple' 
                        : 'bg-secondary border-border text-muted-foreground'
                    }`}>
                      {item.category}
                    </span>
                    {isActive && (
                      <CornerDownLeft className="h-3.5 w-3.5 text-brand-purple animate-pulse shrink-0" strokeWidth={2.25} />
                    )}
                  </div>
                </button>
              );
