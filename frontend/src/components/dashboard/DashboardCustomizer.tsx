  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden select-none">
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        {/* Drawer Panel */}
        <div className="w-screen max-w-md bg-brand-bg border-l border-border-default text-text-muted flex flex-col  animate-in slide-in-from-right duration-300">
          
          {/* Header */}
          <div className="p-5 border-b border-border-default flex items-center justify-between bg-surface-2/50 dark:bg-ink/50">
            <div className="flex items-center space-x-2.5">
              <div className="h-9 w-9 rounded-xl bg-accent-color/15 flex items-center justify-center border border-brand-accent/25">
                <Settings2 className="h-4.5 w-4.5 text-accent-color" />
              </div>
              <div>
                <h3 className="font-semibold text-sm text-text-primary">Customize Dashboard</h3>
                <p className="text-[10px] text-text-muted font-bold mt-0.5 uppercase tracking-wider">Configure Workspace Layout</p>
              </div>
            </div>
            <button 
              onClick={onClose}
              className="p-1.5 rounded-lg hover:bg-surface-2 dark:hover:bg-text-primary text-text-muted hover:text-text-muted transition-colors"
            >
                ...
                return (
                  <div 
                    ...
                    className={`p-4 border rounded-xl flex items-start justify-between gap-4 cursor-pointer transition duration-200 select-none ${
                      isChecked 
                        ? 'border-brand-accent/35 bg-accent-color/[0.03] dark:bg-accent-color/[0.01]' 
                        : 'border-border-default hover:border-border-default bg-surface-2/20'
                    }`}
                  >
                    <div className="flex items-start space-x-3.5 min-w-0">
                      <div className={`mt-0.5 h-8 w-8 rounded-lg flex items-center justify-center shrink-0 border ${
                        isChecked 
                          ? 'bg-accent-color/10 border-brand-accent/20 text-accent-color' 
                          : 'bg-surface-2 dark:bg-text-primary border-border-default text-text-muted'
                      }`}>
                        <Icon className="h-4 w-4" />
                      </div>
                      <div className="min-w-0">
                        <h4 className="text-xs font-semibold text-text-primary">{widget.name}</h4>
                        <p className="text-[10px] text-text-muted mt-1 font-medium leading-normal">{widget.desc}</p>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className={`h-7 w-7 rounded-lg flex items-center justify-center shrink-0 transition-colors border ${
                        isChecked 
                          ? 'bg-accent-color text-surface-0 border-transparent' 
                          : 'bg-surface-2 hover:bg-surface-2 dark:bg-text-primary border-border-default text-text-muted hover:text-text-muted'
                      }`}
          ...
          {/* Footer controls */}
          <div className="p-4 border-t border-border-default bg-surface-2/50 dark:bg-ink/50 flex justify-end">
            <button
              onClick={onClose}
              className="px-4 py-2 bg-accent-color hover:bg-accent-color/90 text-surface-0 rounded-lg text-xs font-bold transition  hover: cursor-pointer"
            >
