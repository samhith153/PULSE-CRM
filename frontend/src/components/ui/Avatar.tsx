...
export default function Avatar({ name, src, size = 'sm', className = '' }: AvatarProps) {
  ...
  return (
    <div
      className={`shrink-0 rounded-full overflow-hidden border border-brand-border-purple/20 bg-brand-accent/10 flex items-center justify-center font-bold text-brand-accent select-none ${SIZES[size]} ${className}`}
      title={name}
    >
      {showImage ? (
