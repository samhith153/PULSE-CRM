    ...
    <div style={{ display: 'block', fontFamily: "'Inter', 'Geist', system-ui, -apple-system, sans-serif", backgroundColor: C.white, color: C.black, minHeight: '100vh', overflowX: 'hidden', width: '100%', boxSizing: 'border-box' }}>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(15,23,42,0.65)', backdropFilter: 'blur(6px)' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: C.white, borderRadius: 20, width: '100%', maxWidth: 380, padding: '28px 32px 24px', boxShadow: '0 32px 80px rgba(0,0,0,0.22)', position: 'relative' }}>
            <button onClick={() => setIsModalOpen(false)} style={{ position: 'absolute', top: 12, right: 12, width: 28, height: 28, borderRadius: 8, border: 'none', background: C.sectionAlt, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.textGray, transition: 'all 0.15s' }}>
              <X size={14} />
            </button>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, background: `linear-gradient(135deg, ${C.violet}, #7c3aed)`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 4px 12px ${C.violet}44` }}>
                <Activity size={15} color={C.white} strokeWidth={2.5} />
              </div>
              <div>
                <span style={{ fontSize: 16, fontWeight: 900, color: C.black, letterSpacing: '-0.02em' }}>Pulse<span style={{ color: C.violet }}>CRM</span></span>
              </div>
            </div>
            <div style={{ marginBottom: 20 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, color: C.black, margin: 0, letterSpacing: '-0.03em', lineHeight: 1.3 }}>{authMode === 'signin' ? 'Welcome back' : 'Get started free'}</h2>
              <p style={{ fontSize: 13, color: C.textGray, fontWeight: 450, margin: '2px 0 0', lineHeight: 1.5 }}>{authMode === 'signin' ? 'Sign in to your account to continue.' : 'Join thousands of teams using PulseCRM.'}</p>
            </div>
            {authError && (
              <div style={{ padding: '10px 14px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 12, color: '#991b1b', fontWeight: 600 }}>{authError}</span>
              </div>
            )}
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {authMode === 'signup' && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: C.textGray, marginBottom: 4 }}>Name</label>
                    <input type="text" value={authName} onChange={e => setAuthName(e.target.value)} placeholder="John" required
                      style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 13, fontFamily: 'inherit', color: C.black, outline: 'none', boxSizing: 'border-box', background: '#faf9ff', transition: 'border-color 0.15s' }}
                      onFocus={e => e.target.style.borderColor = C.violet} onBlur={e => e.target.style.borderColor = C.border} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: C.textGray, marginBottom: 4 }}>Company</label>
                    <input type="text" value={authOrg} onChange={e => setAuthOrg(e.target.value)} placeholder="Acme Inc." required
                      style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 13, fontFamily: 'inherit', color: C.black, outline: 'none', boxSizing: 'border-box', background: '#faf9ff', transition: 'border-color 0.15s' }}
                      onFocus={e => e.target.style.borderColor = C.violet} onBlur={e => e.target.style.borderColor = C.border} />
                  </div>
                </div>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@company.com" required
                    style={{ width: '100%', padding: '10px 12px 10px 36px', borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 13, fontFamily: 'inherit', color: C.black, outline: 'none', boxSizing: 'border-box', background: '#faf9ff', transition: 'border-color 0.15s' }}
                    onFocus={e => e.target.style.borderColor = C.violet} onBlur={e => e.target.style.borderColor = C.border} />
                </div>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: C.textGray, marginBottom: 4 }}>Password</label>
                <div style={{ position: 'relative' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 2 }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: C.textMuted, textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>I am a</p>
                <div style={{ display: 'flex', gap: 4 }}>
                  {(['admin', 'manager', 'representative'] as Role[]).map(r => (
                    <button key={r} type="button" onClick={() => setSelectedRole(r)}
                      style={{ padding: '5px 12px', borderRadius: 20, border: `1.5px solid ${selectedRole === r ? C.violet : C.border}`, background: selectedRole === r ? C.violetLighter : 'transparent', cursor: 'pointer', fontSize: 10, fontWeight: 700, color: selectedRole === r ? C.violet : C.textGray, fontFamily: 'inherit', transition: 'all 0.15s' }}>
              ...
              <button type="submit" disabled={isLoading}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px', background: isLoading ? '#9b72f0' : `linear-gradient(135deg, ${C.violet}, #7c3aed)`, color: C.white, fontSize: 14, fontWeight: 700, borderRadius: 12, border: 'none', cursor: isLoading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', boxShadow: `0 6px 24px ${C.violet}55`, transition: 'all 0.15s', letterSpacing: '-0.01em' }}>
                {isLoading ? <><Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} /> {authMode === 'signin' ? 'Signing in...' : 'Creating account...'}</> : authMode === 'signin' ? 'Sign In' : 'Create Account'}
              </button>
            </form>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '16px 0' }}>
              <div style={{ flex: 1, height: 1, background: C.border }} />
              <span style={{ fontSize: 11, color: C.textMuted, fontWeight: 500, whiteSpace: 'nowrap' }}>or continue with</span>
              <div style={{ flex: 1, height: 1, background: C.border }} />
            </div>
            <button onClick={() => addToast('Google sign-in is currently unavailable.', 'error')}
              style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '11px', background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 12, fontSize: 13, fontWeight: 700, color: C.black, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }}>
            ...
            <p style={{ textAlign: 'center', fontSize: 12, color: C.textGray, margin: '16px 0 0' }}>
