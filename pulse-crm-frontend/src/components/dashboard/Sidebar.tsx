              ...
              className={`w-full flex items-center ${collapsed ? 'justify-center' : 'space-x-3'} px-3 py-2.5 rounded-lg text-xs font-bold transition-all duration-200 cursor-pointer group relative ${
                collapsed 
                  ? (isTabActive('dashboard')
                    ? 'bg-brand-accent/15 text-brand-accent ring-2 ring-brand-accent/30 shadow-sm' 
                    : 'hover:bg-slate-50 text-brand-text/80 hover:text-brand-text')
                  : (isTabActive('dashboard')
            ...
            >
              <LayoutDashboard 
                className={`h-4.5 w-4.5 shrink-0 transition-colors ${
                  isTabActive('dashboard') ? 'text-brand-accent' : 'text-brand-text/70 group-hover:text-brand-text'
                }`}
                strokeWidth={2}
              />
              {!collapsed && (
              ...
              {section.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon 
                      className={`h-4.5 w-4.5 shrink-0 transition-colors ${
                        active ? 'text-brand-accent' : 'text-brand-text/70 group-hover:text-brand-text'
                      }`}
                      strokeWidth={2}
                    />
